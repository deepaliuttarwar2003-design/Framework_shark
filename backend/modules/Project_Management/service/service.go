...
func (s *Service) Create(input dto.CreateProjectManagementDTO) (model.ProjectManagement, error) {
	...
	assignDate, err := time.Parse("2006-01-02", input.DateOfAssign)
	if err != nil {
		return model.ProjectManagement{},
			fmt.Errorf("invalid assign date format")
	}

	completionDate, err := time.Parse("2006-01-02", input.DateOfCompletion)
	if err != nil {
		return model.ProjectManagement{},
	...
	entity := model.ProjectManagement{
		...
		DateOfAssign:     assignDate.Format("2006-01-02"),
		DateOfCompletion: completionDate.Format("2006-01-02"),
		Budget:           input.Budget,
		Status:           input.Status,
	}
