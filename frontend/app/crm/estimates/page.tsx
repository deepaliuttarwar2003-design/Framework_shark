...
} from "@/modules/crm/slices/estimateApiSlice";
...
const localStorageMiddleware =
  (storeApi: any) => (next: any) => (action: any) => {
    ...
    if (
      addEstimate.match(action) ||
      updateEstimate.match(action) ||
      deleteEstimate.match(action)
    ) {
      const state = storeApi.getState();

      if (typeof window !== "undefined") {
        localStorage.setItem(
          "local_redux_estimates",
          JSON.stringify(state.estimates.list)
        );
      }
    }
...
// ==========================================
// 3. MAIN COMPONENT EXPORT WITH PROVIDER
// ==========================================
export default function PoweredEstimatesPage() {
  return (
...
function EstimatesDashboard() {
 ...
 useEffect(() => {
  if (estimatesData) {
    dispatch(hydrateEstimates(estimatesData));
  }
}, [estimatesData, dispatch]);
  // Hydrate estimates from localStorage on mount
  useEffect(() => {
    const cachedData = localStorage.getItem('local_redux_estimates');
    if (cachedData) {
      try {
