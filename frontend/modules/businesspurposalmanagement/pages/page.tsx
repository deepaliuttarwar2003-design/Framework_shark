...
export default function BusinesspurposalmanagementPage() {
  const [data, setData] = useState<Item[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/businesspurposalmanagement")
      .then(res => res.json())
      .then(res => {
        setData(res.data);
        setLoading(false);
    ...
    <div className="min-h-screen bg-black text-slate-200 font-sans p-8">
      ...
      <header className="max-w-5xl mx-auto border-b border-blue-900/50 pb-6 mb-10">
        <h1 className="text-4xl font-bold tracking-tight text-white">
          Businesspurposalmanagement <span className="text-blue-500">Module</span>
        </h1>
